...
public static class QueryDefinitions
{
    ...
    public static readonly string GetPlanningStageTypeRelations = @"
        WITH RECURSIVE split AS (
          SELECT
            ...
            TRIM(SUBSTRING_INDEX(COALESCE(p.compatibel_met, ''), ',', 1)) AS token,
            CASE
              WHEN INSTR(COALESCE(p.compatibel_met, ''), ',') = 0 THEN ''
              ELSE SUBSTR(COALESCE(p.compatibel_met, ''), INSTR(COALESCE(p.compatibel_met, ''), ',') + 1)
            END AS rest
          FROM amusing.ah_podia_typen p
          WHERE p.aktief = 1
            AND COALESCE(p.compatibel_met, '') <> ''
    ...
    public static readonly string LogStageTypeActions = @"
        INSERT INTO amusing.user_log
