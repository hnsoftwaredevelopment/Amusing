...
namespace Amusing.Components.Pages;
    ...
    public class MailingsRecipientsBase : ComponentBase
    {
        ...
        protected List<OperatorsModel> EqualOnlyOperator  = new List<OperatorsModel>
        {
            new OperatorsModel { Value = "equal", Text = "Is gelijk aan" }
        };
        protected List<OperatorsModel> NumberOperator = new List<OperatorsModel>()
        {
        ...
        protected SfQueryBuilder<RuleModel>? queryBuilder;
        protected SfToast? ToastObj { get; set; }
        protected List<string> PersonRoles = new() { "contactpersoon1", "contactpersoon2", "dirigent", "muzikant", "penningmeester", "zanger" };
        ...
        protected string[] YesNoValues = new string[] { "Ja", "Nee" };
        ...
        protected async Task Save()
        {
            ...
            else
            {
                ...
                RecipientsList = await MailingService.GetRecipientListsAsync();
                await Task.Delay( 50 );
                if (GridRef != null) await GridRef.Refresh();
            }
        }
        ...
        protected void OnQueryChanged(Syncfusion.Blazor.QueryBuilder.ChangeEventArgs args)
        {
        ...
        protected async Task ExportToExcel()
        {
            ...
            try
            {
                if (HiddenGridRef != null) await HiddenGridRef.ExportToExcelAsync( exportProps );
        ...
        protected async Task ExportToCsv()
        {
            ...
            try
            {
                if (HiddenGridRef != null) await HiddenGridRef.ExportToCsvAsync( exportProps );
        ...
        protected async Task ExportToPdf()
        {
            ...
            try
            {
                if (HiddenGridRef != null) await HiddenGridRef.ExportToPdfAsync( exportProps );
        ...
        protected async Task GenerateMailList()
        {
            ...
            var fullQuery = QueryBuilderHelper.DetermineQueryFromRules(rules, SourceChecked);
            var result = await MailingService.GetDynamicRecipientsAsync(fullQuery) ?? [];
            ...
            foreach ( var row in result )
            {
                ...
                if ( dict.TryGetValue( "Aktief", out var aktiefValue ) && aktiefValue is bool b )
                    dict [ "Aktief" ] = b ? "Ja" : "Nee";

                foreach ( var key in new [ ] { "Infomailing","Ingeschreven","Afgehaakt","Betaald","Bevestigd", "Kleedkamer","SingAlong","Stand","Beoordeling","Vrijwilliger" } )
                {
                    if ( dict.TryGetValue( key, out var val ) && val is int i )
                        dict [ key ] = i == 1 ? "Ja" : "Nee";
                }
            }
        ...
        protected ExpandoObject ConvertValuesToJaNee( ExpandoObject row )
        {
            ...
            var newDict = (IDictionary<string, object>)newRow;
